          ...
          <div class="card card-weather">
            ...
            <div class="card-body p-0">
              <div class="d-flex weakly-weather">
                <div class="weakly-weather-item" v-for="(day, index) in fiveDay" :key="index">
                  <p class="mb-0">{{ day.Date.substring(5, 10) }}</p>
                  <i class="mdi mdi-weather-cloudy"></i>
                  <p class="mb-0">
                    {{((((day.Temperature.Maximum.Value + day.Temperature.Minimum.Value) / 2) - 32) * 5 / 9).toFixed(2)}}
                    <span class="symbol">°</span>C
                  </p>
                </div>
                <input type="submit" @click="mapView" class="m-2 btn btn-secondary" value="View Map" />
...
export default {
  ...
  mounted() {
    let self = this;
    axios
      .post("/conn/weather/get-location-key", {
        location: "Cape Town, Western Cape"
      })
      .then(response => {
        self.fiveDay = response.data.fiveDay.DailyForecasts;
        console.log(self.fiveDay);
      });
  },
  methods: {
    mapView() {
